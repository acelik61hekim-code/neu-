...
function storySummary(
  ...
  return [
    ...
    `Schlusspunkt: ${readString(
      moviePlan.finalPayoff,
    )}`,
  ]
    .filter(
      (
...
function cleanVoiceoverText(
  ...
  return value
    ...
    .replace(
      /^(?:voice-?over|sprechertext|text)\s*:\s*/i,
      "",
    )
    .trim();
}
...
export async function POST(
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Die automatische Sprechertext-Erstellung ist momentan nicht verfügbar.",
      },
      {
        status:
          503,
  ...
  if (
    typeof targetDurationSeconds !==
      "number" ||

    !SUPPORTED_DURATIONS.includes(
      targetDurationSeconds as
    ...
    return NextResponse.json(
      {
        error:
          "Für diese Videolänge kann kein Sprechertext erstellt werden.",
      },
      {
        status:
          400,
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Erstelle zuerst deine Geschichte mit dem AI Director.",
      },
      {
        status:
          400,
  ...
  /*
   * Die Wortzahl wird direkt aus der
   * tatsächlichen Videolänge berechnet.
   *
   * Dadurch funktioniert dieselbe Logik
   * korrekt für:
   *
   * 15 s
   * 30 s
   ...
   * 120 s
   *
   * und weiterhin für alte 8-s-Legacy-Aufträge.
   */
  const targetWords =
    Math.max(
      14,
  ...
  try {
    ...
    const response =
      await ai.models
        .generateContent({
          ...
          contents:
            `Create the exact voice-over narration for this video.\n\n${summary}`,
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Der automatische Sprechertext war nicht passend. Bitte versuche es erneut.",
        },
        {
          status:
            502,
    ...
    console.error(
      "Automatische Voice-over-Erstellung fehlgeschlagen:",
      error,
    );
    ...
    return NextResponse.json(
      {
        error:
          "Der Sprechertext konnte gerade nicht automatisch erstellt werden.",
      },
      {
        status:
          502,
