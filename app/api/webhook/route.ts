...
async function waitForClip(operationName: string): Promise<string> {
  for (let attempt = 0; attempt < 36; attempt++) {
    // Nur alle 10 Sekunden prüfen (spart Requests)
...
async function generateShortVideoInBackground(
  jobId: string,
  prompt: string
) {
...
async function generateLongVideoInBackground(
  jobId: string,
  prompt: string
) {
  const scenes = await splitIntoScenes(
    prompt,
    LONG_FORMAT_SCENE_COUNT
  );
  ...
  const videoUrls = await runWithConcurrencyLimit(
    scenes,
    1,
    async (scenePrompt) => {
      const operationName =
        await startVideoGeneration(scenePrompt);

      const videoUrl = await waitForClip(operationName);

      // 30 Sekunden Pause bis zur nächsten Veo-Anfrage
